...
from dataset.voc import VOC_CLASSES, VOC_CLASSES_INDEX
...
def visualize(img, bboxes, scores, labels, class_colors, vis_thresh=0.3):
    ...
    for i, bbox in enumerate(bboxes):
        if scores[i] > vis_thresh:
            ...
            cls_id = VOC_CLASSES_INDEX[int(labels[i])]#cls_id = coco_class_index[int(labels[i])]
            mess = '%s: %.2f' % (VOC_CLASSES[cls_id], scores[i])#mess = '%s: %.2f' % (coco_class_labels[cls_id], scores[i])

...
from models.yolov7.yolov7_basic import RepNCSP, RepNCSPELAN4

try:
    from .yolov7_basic import Conv, ELANBlock, DownSample, Conv9
except:
...
class ELANNet_Lagre(nn.Module):
    def __init__(self, norm_type='BN', depthwise=False):
        act_type = 'silu'
        ...
        ## P2/4: Stage-1
        self.layer_2 = nn.Sequential(
            Conv9(64, 128, k=3, s=2),
            RepNCSPELAN4(128,256,128,64,1)
        )
        ## P3/8: Stage-2
        self.layer_3 = nn.Sequential(
            Conv9(256,256, k=3 , s=2),
            RepNCSPELAN4(256,512,256,128,1)
        )
        ## P4/16: Stage-3
        self.layer_4 = nn.Sequential(
            Conv9(512,512, k=3, s=2),
    ...
    def forward(self, x):
        ...
        c3 = self.layer_3(c2)
        c4 = self.layer_4(c3)
        c4_1 = self.layer_4_1(c4)
        c5 = self.layer_5(c4)
        c5_1 = self.layer_5_1(c5)

        outputs = [c3, c4_1, c5_1]
...
def build_backbone(cfg, pretrained=False):
    ...
    if cfg['backbone'] == 'elannet_huge':
        ...
        backbone = ELANNet_Lagre(cfg['bk_act'], cfg['bk_norm'])
